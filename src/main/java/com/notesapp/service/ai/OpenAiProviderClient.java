...
public class OpenAiProviderClient implements AiProviderClient {
    ...
    public OpenAiProviderClient(AiProperties aiProperties, AiBudgetGuard aiBudgetGuard,
    ...
    private <T> T runWithBudget(SupplierWithException<T> supplier) {
        if (aiProperties.getOpenAiApiKey() == null || aiProperties.getOpenAiApiKey().isBlank()) {
            log.warn("OpenAI provider selected but API key not configured. Falling back to deterministic provider.");
    ...
    private <T> T fallbackValue(SupplierWithException<T> supplier) {
        try {
            ...
            throw new IllegalStateException("OpenAI provider call failed", e);
        }
    }
