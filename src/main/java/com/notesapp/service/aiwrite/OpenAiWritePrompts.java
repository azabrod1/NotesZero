...
final class OpenAiWritePrompts {
    ...
    static final String ROUTER_PROMPT_VERSION = "openai-router-v2";
    static final String PLANNER_PROMPT_VERSION = "openai-planner-v3";
    ...
    static String routeInstructions() {
        return """
            ...
            - Use short snake_case codes such as selected_note_hint, retrieval_note_match, explicit_question, ambiguous_capture, selected_notebook_hint, rewrite_request.
            ...
            Best result: route to the family notebook, prefer WRITE_EXISTING_NOTE if one family note clearly fits, otherwise CREATE_NOTE with DIRECT_APPLY. Do not use NOTEBOOK_INBOX for this concrete observation.
    ...
    static String plannerInstructions() {
        return """
            ...
            Task:
            Convert one user message into the smallest valid patch plan for a canonical note document.
            ...
            - Use APPEND_SECTION_CONTENT for additive updates.
            - Use REPLACE_SECTION_CONTENT only when the user explicitly asks to rewrite, correct, or replace content.
            - Use REPLACE_NOTE_OUTLINE only when the user explicitly asks to rewrite, reorganize, or clean up the whole note.
            - Never delete sections or reorder the outline unless the user explicitly asks.
            ...
            Best result: one timeline op for the shipped event, one status op for the blocker, and one task op for the resend logging follow-up.
    ...
    static String plannerInput(PatchRequestContext context) {
        ...
        if (CanonicalNoteTemplates.PROJECT_NOTE.equals(context.routePlan().targetNoteType())) {
